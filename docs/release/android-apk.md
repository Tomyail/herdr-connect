...
这些文件会附加到对应 tag 的 GitHub Release；preview tag 对应 prerelease。首次安装 APK 时，Android 可能要求用户允许浏览器或文件管理器“安装未知应用”。
...
当前 `apps/mobile/app.config.ts` 已包含 Android package name、`versionCode` 和局域网发现权限；本地 config plugin 会为当前 HTTP MVP 显式启用 cleartext traffic。每次发布新版本前必须递增 `android.versionCode`。
